...
**Launch flags, resolved in code (Aug 13):** the mood tap is measured (`mood_response` event — the number the paid tier lives on); §4.1's own Day-7 test is finally wired (`intention_path` user property, segment Firebase's retention cohorts by it); widget completions are tagged `source: widget` and get a quiet mood catch-up card on the next open (max 3, 48h lookback); JSON export lives in the profile ("your moments are yours" now has a door); and the 1000-cap trim sorts before it drops, so retro-logs can no longer push recent moments off the tail.

**Yours, not code:** ① Firestore rules — verify `backups/{uid}` is readable/writable only by `request.auth.uid == uid` in the Firebase console before strangers sign in. ② Trial length — the 7-day window shows themes + season + (usually) the letter, but never a plan; if trial→paid lands under the ~62% benchmark, the cheapest lever is a 14-day intro offer in ASC, no code. ③ The ASC price rows and the real-device pass, as before.
